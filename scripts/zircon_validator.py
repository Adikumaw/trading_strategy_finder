...
# --- Configurations are unchanged and correct ---
...
def simulate_trades(strategy, entries, full_silver_data, market_config):
    """UPGRADED: Returns both performance stats AND the detailed trade log."""
    ...
    for entry in entries.itertuples():
        ...
        # Definitive fix for sell trade SL/TP logic
        ...
        # Using a simple but safe lookforward limit
...
def run_validation_for_strategy(strategy, market_data_cache, market_csv, trade_logs_dir):
    """Backtests a single strategy and saves its trade log."""
...
if __name__ == "__main__":
    ...
    try: choice = int(input(f"Enter number (1-{len(master_strategy_files)}): ")) - 1; file_to_validate = master_strategy_files[choice]
    ...
    # Ensure trade_type is correctly loaded, not inferred
    if 'trade_type' not in strategies_df.columns:
        print("❌ 'trade_type' column not found in master strategies file. Please re-run the Diamond layer with the bug fix."); exit()
    ...
    # --- LOGIC CHANGE: Include the origin market in the test list ---
    ...
    # Resumability logic remains the same
