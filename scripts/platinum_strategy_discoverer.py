...
def get_rules_from_tree(tree, feature_names):
    ...
    def recurse(node, rule_path):
        if tree_.feature[node] != _tree.TREE_UNDEFINED:
            ...
            predicted_density = tree_.value[node][0][0]
...
if __name__ == "__main__":
    ...
    if not combination_files:
        ...
        for fname in combination_files:
            ...
            # --- NEW: Define a path for the processed log ---
            ...
            try:
                ...
                # --- NEW: Load the set of already processed definition keys ---
                try:
                    with open(processed_log_path, 'r') as f:
                        processed_keys = set(f.read().splitlines())
                ...
                # Create a unique key for each definition
                ...
                # --- NEW: Determine definitions to process based on the log file ---
                # Process if NOT in the log, OR if it IS in the blacklist
                definitions_to_process = all_definitions[
                    (~all_definitions['key'].isin(processed_keys)) | 
                    (all_definitions['key'].isin(blacklist['key'] if not blacklist.empty else []))
                ]
                ...
                with open(processed_log_path, 'a') as log_file:
                    for _, definition in tqdm(definitions_to_process.iterrows(), total=len(definitions_to_process), desc="Discovering Strategies"):
                        # --- NEW: Log the key BEFORE processing ---
                        ...
                        slice_df = pd.concat([filter_chunk_for_definition(chunk, definition) for chunk in outcomes_iterator])
                ...
                try:
                    ...
                    final_df.drop_duplicates(subset=['type', 'sl_def', 'sl_bin', 'tp_def', 'tp_bin', 'market_rule'], keep='last', inplace=True)
