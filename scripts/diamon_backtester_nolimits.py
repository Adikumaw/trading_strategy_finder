# diamond_backtester_nolimits.py
...
# --- CORE FUNCTIONS (All correct and unchanged) ---
# ... (All helper functions from the previous script are included here) ...
...
# --- SPEED OPTIMIZATION: New function designed for parallel execution ---
def run_backtest_for_strategy(strategy, market_data_cache, markets_to_test):
    """
    Worker function for multiprocessing. Backtests a SINGLE strategy across ALL markets.
    """
    all_market_results = []
...
if __name__ == "__main__":
    ...
    print(f"\nLoaded top {len(strategies_df)} strategies for multi-market backtesting on {len(markets_to_test)} market(s).")

    # --- Pre-process all market data ONCE (Unchanged) ---
    ...
    # --- SPEED OPTIMIZATION: Main Backtesting Loop using Multiprocessing ---
    all_results_nested = []
    
    # Convert strategies DataFrame to a list of dicts for the pool
    strategy_list = strategies_df.to_dict('records')
    
    num_processes = min(MAX_CPU_USAGE, len(strategy_list))
    print(f"\n🚀 Starting parallel backtesting with {num_processes} workers...")
    
    with Pool(processes=num_processes) as pool:
        # We use partial to "freeze" the market_data_cache and markets_to_test arguments for the worker function
        func = partial(run_backtest_for_strategy, market_data_cache=market_data_cache, markets_to_test=markets_to_test)
        # tqdm shows the progress as strategies are completed
    ...
    # --- Save Results (Logic is now cleaner) ---
    if all_detailed_results:
        ...
        # Check for failed markets to build the blacklist
        passed_strategies = detailed_df[
            (detailed_df.profit_factor > PROFIT_FACTOR_PASS_THRESHOLD) & 
            (detailed_df.max_drawdown_pct < MAX_DRAWDOWN_PASS_THRESHOLD)
        ].groupby('strategy_id')['market'].count()

        summary_df['markets_passed'] = summary_df['strategy_id'].map(passed_strategies).fillna(0).astype(int).astype(str) + '/' + summary_df['markets_tested'].astype(str)
        
        # Any strategy that didn't pass on all markets is a candidate for blacklisting
        failed_strategy_ids = summary_df[summary_df['markets_passed'].apply(lambda x: eval(x) < 1.0)]['strategy_id'].unique()
        
        strategy_defs = detailed_df.drop(columns=['market', 'profit_factor', 'max_drawdown_pct', 'final_capital', 'total_trades', 'win_rate_pct', 'session_pct', 'trend_regime_pct', 'vol_regime_pct']).drop_duplicates(subset='strategy_id')
