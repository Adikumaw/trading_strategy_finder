...
def robust_read_csv(filepath):
    try: df = pd.read_csv(filepath, sep=None, engine="python", header=0)
    except Exception: df = pd.read_csv(filepath, sep=None, engine="python", header=None)
    if df.shape[1] > 5:
        df = df.iloc[:, :6]; df.columns = ['time', 'open', 'high', 'low', 'close', 'volume']
    elif df.shape[1] == 5:
...
def add_all_features(df):
    for p in SMA_PERIODS: df[f"SMA_{p}"] = ta.trend.SMAIndicator(df["close"], p).sma_indicator()
    for p in EMA_PERIODS: df[f"EMA_{p}"] = ta.trend.EMAIndicator(df["close"], p).ema_indicator()
    ...
    if "volume" in df.columns: df["OBV"] = ta.volume.OnBalanceVolumeIndicator(df["close"], df["volume"]).on_balance_volume()
    patterns_data = {};
    for p in talib.get_function_groups().get("Pattern Recognition", []):
        try: patterns_data[p] = getattr(talib, p)(df["open"], df["high"], df["low"], df["close"])
        except: continue
    ...
    for i in range(lookback + 2, n - 2):
        ...
        if not np.isnan(last_support):
            ...
            for j in range(max(0, i - lookback), i):
                if lows[j] < last_support:  # support broken
                    valid = False
                    break
            if valid:
        ...
        if not np.isnan(last_resistance):
            valid = True
            for j in range(max(0, i - lookback), i):
                if highs[j] > last_resistance:  # resistance broken
                    valid = False
                    break
            if valid:
...
def add_support_resistance(df, lookback=SR_LOOKBACK):
    ...
    # --- Optional: smooth with forward fill for plotting only ---
...
# def add_htf_features(df, htf_filepath):
#     print(f"  Adding HTF features from: {os.path.basename(htf_filepath)}")
#     htf_df = robust_read_csv(htf_filepath)
#     if htf_df.empty: return df
#     htf_df['SMA_50_HTF'] = ta.trend.SMAIndicator(htf_df["close"], 50).sma_indicator()
#     htf_df['RSI_14_HTF'] = ta.momentum.RSIIndicator(htf_df["close"], 14).rsi()
#     htf_features = htf_df[['time', 'SMA_50_HTF', 'RSI_14_HTF']].copy()
#     return pd.merge_asof(df.sort_values('time'), htf_features.sort_values('time'), on='time', direction='backward')
...
def create_silver_data_chunked(bronze_file, raw_file, silver_file, htf_file=None):
    ...
    bronze_reader = pd.read_csv(bronze_file, chunksize=BRONZE_CHUNK_SIZE)
    is_first_chunk = True
    ...
    for chunk in tqdm(bronze_reader, desc="Merging Bronze Chunks"):
        ...
        # --- FIX: Filter the bronze chunk and create an explicit copy to avoid the warning ---
    ...
    if not is_first_chunk: # This means at least one chunk was written
